...
{
    public class AstPostDecrement : AstStatement
    {
        public IAstExpression AstExpression;

        public AstPostDecrement(
            string path, int lineNumber, int columnNumber)
            : base(path, lineNumber, columnNumber) { }

        public AstPostDecrement(IParser parser) : base(parser) { }
        public AstPostDecrement(IParser parser, bool useLookAhead) : base(parser, useLookAhead) { }

        public override string ToString()
        {
            StringBuilder sb = new StringBuilder();
            sb.Append("--AstPostDecrement--{0}{0}");
